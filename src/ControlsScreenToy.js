...
// Staggered float animation for toy keys
const createFloatAnimation = (delay = 0) => ({
  y: [0, -2, 0, 2, 0],
  transition: {
    duration: 2.5,
    repeat: Infinity,
...
const ToyKeyCap = ({ children, wide = false, className = '', floatDelay = 0 }) => {
  const [isPressed, setIsPressed] = useState(false);
  
  return (
    <motion.div
      className={`toy-key ${wide ? 'toy-key--wide' : ''} ${className}`}
      animate={createFloatAnimation(floatDelay)}
      whileHover={{
        filter: 'brightness(1.1)',
      }}
      whileTap={{
        y: 4,
        boxShadow: '0 2px 0 #3d1045, 0 3px 6px rgba(0,0,0,0.2)',
    ...
    >
      <span className="toy-key__label">{children}</span>
      <div className="toy-key__shine" />
    </motion.div>
  );
};
...
const SpriteContainer = ({ children, isHovered = false }) => {
  return (
    <motion.div
      className="sprite-container-toy"
...
const ControlsScreenToy = () => {
  return (
    <motion.div
      className="controls-screen-toy"
    ...
    >
      ...
      <ControlSection>
        ...
        <div className="controls-row-toy">
          {/* WASD Cluster */}
          <div className="key-cluster-toy">
            <div className="key-row-toy key-row-toy--centered">
              <ToyKeyCap floatDelay={0}>W</ToyKeyCap>
            </div>
            <div className="key-row-toy">
              <ToyKeyCap floatDelay={0.15}>A</ToyKeyCap>
              <ToyKeyCap floatDelay={0.3}>S</ToyKeyCap>
              <ToyKeyCap floatDelay={0.45}>D</ToyKeyCap>
            </div>
          </div>

          <span className="or-divider-toy">OR</span>

          {/* Arrow Keys Cluster */}
          <div className="key-cluster-toy">
            <div className="key-row-toy key-row-toy--centered">
              <ToyKeyCap floatDelay={0.1} className="arrow-key-toy">↑</ToyKeyCap>
            </div>
            <div className="key-row-toy">
              <ToyKeyCap floatDelay={0.25} className="arrow-key-toy">←</ToyKeyCap>
              <ToyKeyCap floatDelay={0.4} className="arrow-key-toy">↓</ToyKeyCap>
              <ToyKeyCap floatDelay={0.55} className="arrow-key-toy">→</ToyKeyCap>
            </div>
          </div>
        </div>
      </ControlSection>
      ...
      <ControlSection>
        ...
        <div className="controls-row-toy">
          <motion.div
            className="mouse-icon-container-toy"
          ...
          <ToyKeyCap wide floatDelay={0.35}>Space</ToyKeyCap>
        </div>
      </ControlSection>
      ...
      <ControlSection>
        ...
        <div className="controls-row-toy">
          <div className="key-row-toy key-row-toy--spaced">
            <ToyKeyCap floatDelay={0}>1</ToyKeyCap>
            <ToyKeyCap floatDelay={0.2}>2</ToyKeyCap>
            <ToyKeyCap floatDelay={0.4}>3</ToyKeyCap>
          </div>
        </div>
      </ControlSection>
    </motion.div>
...
export default ControlsScreenToy;
