...
const CreateMapModal = ({ onClose, onMapGenerated, getMapButtonPosition, onFlyComplete, initialTab = 'create' }) => {
  ...
  const [closeAnimation, setCloseAnimation] = useState({ x: 0, y: 0, scale: 1 });
  ...
  // Fly-to-button close animation
  const handleCloseWithFly = useCallback(async () => {
    if (isClosing) return;
    
    const buttonPos = getMapButtonPosition?.();
    const isLowEnd = isLowEndDevice();
    
    if (!buttonPos || !modalRef.current) {
    ...
    const translateX = buttonPos.x - modalCenterX;
    const translateY = buttonPos.y - modalCenterY;
    ...
    if (isLowEnd) {
      ...
      setCloseAnimation({ x: translateX, y: translateY, scale: 0.15 });
      await new Promise(resolve => setTimeout(resolve, 300));
    } else {
      // Full animation
      setCloseAnimation({ x: translateX, y: translateY, scale: 0.15 });
    ...
    // Trigger button catch effect
    onFlyComplete?.();
    
    // Small delay before actual close
    await new Promise(resolve => setTimeout(resolve, 100));
    
    onClose();
  }, [isClosing, getMapButtonPosition, onClose, onFlyComplete]);
  ...
  // Handle overlay click - use fly animation if we have a generated map
  ...
  return (
    ...
    >
      <motion.div
        ref={modalRef}
        className={`modal-container ${generationPhase === 'tension' ? 'modal-container--generating' : ''} ${isClosing ? 'modal-container--closing' : ''}`}
        ...
        animate={isClosing 
          ? { 
              ...
              opacity: closeAnimation.scale < 0.5 ? 0 : 1,
            }
      ...
      >
        ...
        <div className={`modal-background-overlay ${detectedTheme ? `modal-background-overlay--${detectedTheme}` : ''}`}>
        ...
        {!isGenerating && (
          <div className="modal-tabs">
            <button
              ...
              className={`modal-tab ${activeTab === 'create' ? 'modal-tab--active' : ''}`}
            ...
            <button
              ...
              className={`modal-tab ${activeTab === 'my-maps' ? 'modal-tab--active' : ''}`}
